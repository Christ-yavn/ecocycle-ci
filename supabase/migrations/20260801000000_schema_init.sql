...
create table if not exists public.confirmations (
  ...
  poids_reel_kg numeric check (poids_reel_kg < 10000), -- [CORRECTIF SÉCURITÉ] Bloque les valeurs aberrantes pour tricher sur les points
  note text,
...
create table if not exists public.point_transactions (
  ...
  constraint point_transactions_lot_motif_key unique (lot_id, motif) -- [CORRECTIF SÉCURITÉ] Empêche le déclenchement infini du trigger sur un même lot
);
...
-- Producteur modifie ses lots (uniquement si le lot n'est pas encore réservé/collecté)
drop policy if exists "lots_update_own" on public.lots;
create policy "lots_update_own" on public.lots
  for update using (auth.uid() = producteur_id and status = 'publie') with check (auth.uid() = producteur_id and status = 'publie'); -- [CORRECTIF SÉCURITÉ]
