...
export default async function CollecteurCartePage() {
  ...
  if (!user) {
    redirect("/login?redirect=/collecteur/carte");
  }
  ...
  // Récupérer les lots publiés avec coordonnées
  ...
  const { data: producteurs } = await supabase
    .from("users")
    .select("id, name")
    .in("id", producteurIds.length > 0 ? producteurIds : ["_"]);
